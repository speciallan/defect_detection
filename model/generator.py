...
from defect_detection.model.pascal_voc import PascalVocGenerator
from defect_detection.model.transform import random_transform_generator
...
def get_generators(images_path, annotations_path, train_val_split, batch_size, classes, img_min_size, img_max_size, shuffle=True, debug=False, transform=True):
    ...
    train_test_split = 1
    total_id = int(len(annotation_files) * train_test_split)
    max_id = int(train_val_split * total_id)

    train_ids = annotation_files[0:max_id]
    if train_val_split < 1.:
        val_ids = annotation_files[max_id:total_id]
    else:
        val_ids = None
